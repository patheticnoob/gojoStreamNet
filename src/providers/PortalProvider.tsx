...
export interface PortalConsumerProps {
  setPortal: (anchor: HTMLElement | null, video: AnimeContent | null) => void;
}
export interface PortalDataConsumerProps {
  anchorElement: HTMLElement | null;
  miniModalMediaData: AnimeContent | null;
}
...
export default function PortalProvider({ children }: { children: ReactNode }) {
  ...
  const [miniModalMediaData, setMiniModalMediaData] = useState<AnimeContent | null>(
    null
  );

  const handleChangePortal = useCallback(
    (anchor: HTMLElement | null, video: AnimeContent | null) => {
